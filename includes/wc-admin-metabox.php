...
function comic_post_meta_box( $post ) {
	...
	<?php } elseif ( $series && !get_the_comic()->file ) { ?>
		...
		<?php
			//Get all the comic files, BUT exclude files ending in -narrow or -narrow-2x
			$comic_files = array_values( preg_grep( '/^((?!narrow*).)*$/', glob( $file_path . '*.*' ) ) );
			...
			<?php
				foreach ( $comic_orphans as $orphan ) {
					...
					if ( 'swf' == $orphan_info[ 'extension' ] ) {
						...
						$preview = '<img src="' . $img . '" alt="' . $orphan_info[ 'basename' ] . '" title="' . $orphan_info[ 'basename' ] . '" style="vertical-align:top;height:100px;" />';
					}
	...
	$file_path = get_comic_directory('abs', false,get_comic_category());
	...
	if($comics->have_posts()): 
		while($comics->have_posts()): 
			$comics->the_post();
			$comic_check = get_the_comic();
			array_push($comic_posts,$file_path. $comic_check->file_name);
		//$total_post_num += 1;
		endwhile;
	endif;
	...
	$filename=get_post_meta($post->ID,'comic_file',true);
	if(!$filename) $filename = $comic->file_name;
	?>
	<p>
		<label for="comic_filename"><strong><?php _e('File','webcomic') ?></strong></label>	<br />
		<?php if($filename):?>
		Currently Selected: <em><?=$filename;?></em><br />
		<?php if($comic && $comic->fallback):?>
		<p class="updated">Update post to reassign comic</p>
		<php? else: ?>
		<input type="checkbox" id="comic_clearcheckbox" name="comic_clearfilename" onclick="if(this.checked){getElementById('comic_filename-label').style.display='block';getElementById('comic_select').style.display='none';getElementById('comic_retainfilename').value='true';getElementById('comic_filename').value='0';}" />
		<a id="comic_clearlabel" href="#" onclick="var elem=getElementById('comic_clearcheckbox');elem.click();">Click to remove comic</a><br />
		<a href="#" id="comic_filename-label"  onclick="getElementById('comic_select').style.display='block';this.style.display='none';getElementById('comic_clearcheckbox').checked=false;getElementById('comic_retainfilename').value='false'">Click here to change comic</a>
		<input type="hidden" name="comic_retainfilename" id="comic_retainfilename" value="true" />
		<?php endif;?>
		<?php endif; if($comic && !$comic->fallback):?>
		<div id="comic_select" <?php if($filename):?>style="display:none"<?php endif; ?>>
			<select id="comic_filename" name="comic_filename" class="dropdown-comics">
			<?php if(count($comic_orphans) > 0):?><option value="0">Select Comic</option>
			<?php foreach($comic_orphans as $file):
					//$file = strrev(end(explode(".", strrev($file),2)));
					$file = pathinfo( $file );
					$file = $file['basename'];
					//$file = substr($file['basename'], 0, strrpos( $file['basename'], '.'));
					print "<option value=\"{$file}\">{$file}</option>\r\n\t\t";
				endforeach;
				else:
				?><option value="0">No orphaned comics...get drawing</option>
				<?php endif; ?>
			</select>
			<p><?php _e('If you are uploading a comic and leave this blank it will be automatically set to the name of the uploaded comic file.','webcomic') ?></p>
		</div>
		<?php else:?><input type="hidden" id="comic_filename" name="comic_filename" value="<?=$filename;?>" /><?php endif;?>
	</p>
	...
	<?php if ( $comic->file && !$comic->fallback ) { $file = pathinfo( get_comic_directory( 'abs', false, $series ) . $comic->file_name ); ?>
		<p style="width:50%">
			<label for="comic_file"><strong><?php _e( 'Rename File', 'webcomic' ); ?></strong></label><br />
	...
	<?php } ?>
		<input type="text" name="comic_description" id="comic_description" style="width:99%" value="<?php echo htmlspecialchars(get_post_meta( $post->ID, 'comic_description', true )); ?>" />
	</p>
	<p><?php _e( 'If provided, the comic description will replace the post title as the hover text for links to and images of the comic...or not, because the old system was crap.', 'webcomic' ); ?></p><br />
	<p>
...
function comic_post_meta_box_save( $id, $post ) {
	...
	if ( 0 === $_FILES[ 'new_comic_file' ][ 'error' ] ) {
		...
		} elseif ( $_REQUEST['comic_filename'] && $_REQUEST['comic_filename'] != get_post_meta ($id, 'comic_file', true) ) {
		
			delete_post_meta($id, 'comic_filename');
			delete_post_meta($id, 'comic_file');
			delete_post_meta($id, 'comic_large');
			delete_post_meta($id, 'comic_medium');
			delete_post_meta($id, 'comic_thumb');
			
			add_post_meta($id, 'comic_file', $_REQUEST['comic_filename']);
			
			$file = pathinfo($file_path . $_REQUEST['comic_filename']);
			if(!$file['filename'])
				$file['filename'] = substr($file['basename'], 0, strlen($file['basename']) - strlen($file['extension'])-1);
			if(is_file($thumb_path.$file['filename'] . '-large.' . $file['extension']))
				add_post_meta($id, 'comic_large', $file['filename'] . '-large.'.$file['extension']);
			if(is_file($thumb_path.$file['filename'] . '-medium.'.$file['extension']))
				add_post_meta($id, 'comic_medium', $file['filename'].'-medium.'.$file['extension']);
			if(is_file($thumb_path.$file['filename'] . '-thumb.'.$file['extension']))
				add_post_meta($id, 'comic_thumb', $file['filename'].'-thumb.'.$file['extension']);
			
		} elseif ($_REQUEST['comic_clearfilename']) {
			delete_post_meta($id, 'comic_filename');
			delete_post_meta($id, 'comic_file');
			delete_post_meta($id, 'comic_large');
			delete_post_meta($id, 'comic_medium');
