...
function get_the_comic( $id = false, $limit = false, $chapter = false ) {
    ...
    $comic_dir = isset( $category ) ? $category : get_post_comic_category( $comic_post->ID );
...
/* EXPIERMENTING RIGHT HERE */  
    $width = $comic->file_data[ 0 ];
    $height = $comic->file_data[ 1 ];
    $alttext= $comic->description;
    $comic_path = pathinfo($comic->file);
    $comic_retina = $comic_path['dirname'] . '/high-res/' . $comic_path['filename']  . '-2x.' . $comic_path['extension'] . ' 2x';
...
echo $path_parts['filename'], "\n"; // since PHP 5.2.0*/
    ...
    $datadims = 'data-width="' . $width . '" data-height="' . $height . '"';
    
    if ( $comic->flash ) {
        if ( 'full' == $size )
            $dims = $comic->file_data[ 3 ];
        else
            $dims = 'data-height="' . get_option( 'comic_' . $size . '_size_h' ) . '" data-width="' . get_option( 'comic_' . $size . '_size_w' ) . '"';
        ...
        $output ='<img srcset="' . $comic_retina . '" img src="' . $size . '" ' . $datadims . ' alt="' . $comic->title . '" title="' . $comic->description . '" class="' . $comic->class . '" />';
    }
...
<picture>
    ...
    <source media="(min-width:478px)" srcset="file.png, high-res/file-2x.png 2x">
    <source media="(min-width:300px)" srcset="file-narrow.png, high-res/file-narrow-2x.png 2x">
    <!-- [if IE 9]></video><![endif]-->
    <img src="file.png" srcset="file-2x.png 2x" alt="" title="" class="">
</picture>
...
function get_the_chapter( $id = false ) {
    ...
    if ( !isset($chapter) || is_wp_error( $chapter ) )
...
function first_comic_link( $label = false, $limit = false, $bookend = false ) {
    ...
    if ( 'thumb' == $label || 'medium' == $label || 'large' == $label || 'full' == $label ) {
        ...
        $link = ( $label ) ? $label : __( 'First comic', 'webcomic' );
    }
    
    $current  = ( ( $limit && $post->ID == $chapterID ) || ( $post->ID == $comic->ID ) ) ? ' current-comic' : '';
    ...
    echo $object . '<a href="' . $comic->link . '" alt="' . $label . '" class="' . $class . '"><span>' . $link . '</span></a>';
}
...
function last_comic_link( $label = false, $limit = false, $bookend = false ) {
    ...
    if ( 'thumb' == $label || 'medium' == $label || 'large' == $label || 'full' == $label ) {
        ...
        $link = ( $label ) ? $label : __( 'Last comic;', 'webcomic' );
    }
...
function previous_comic_link( $label = false, $limit = false, $bookend = false ) {
    ...
    if ( 'thumb' == $label || 'medium' == $label || 'large' == $label || 'full' == $label ) {
        ...
        $link = ( $label ) ? $label : __( 'Previous comic', 'webcomic' );
    }
...
function next_comic_link( $label = false, $limit = false, $bookend = false ) {
    ...
    if ( 'thumb' == $label || 'medium' == $label || 'large' == $label || 'full' == $label ) {
        ...
        $link = ( $label ) ? $label : __( 'Next comic', 'webcomic' );
    }
    ...
    echo $object . '<a href="' . $comic->link . '" title="' . $label . '" class="' . $class . '"><span>' . $link . '</span></a>';
}
...
function random_comic_link( $label = false, $limit = false, $chapter = false ) {
    ...
    if ( 'thumb' == $label || 'medium' == $label || 'large' == $label || 'full' == $label ) {
        ...
        $link = ( $label ) ? $label : __( 'random_link', 'webcomic' );
    }
